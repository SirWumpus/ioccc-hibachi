...
class Date 
{
	...
	/*
	 * $obj->getSmallCalendar([$cellContentFunction[, $year, $month]])
	 * Date::getSmallCalendar($cellContentFunction, $year, $month)
	 */
	function getSmallCalendar(/* $cellContentFunction [, $year, $month] */)
	{
		switch (func_num_args()) {
		case 3:
			$cellContentFunction = func_get_arg(0);
			$year = func_get_arg(1);
...
EOT;
		...
		for ($day = 1; $day <= $daysInMonth; $day++, $weekDay = ($weekDay + 1) % 7) {
			if (1 < $day && $weekDay == 0) {
			...
			if ($cellContentFunction == NULL)
				$content = "<span class='smCalendarNumber'>${day}</span>";
			else
				$content = $cellContentFunction($year, $month, $day);
							
			$rc .= sprintf(
				"\t<td class='smCalendarCell %s%s' valign='top'>%s</td>\n", 
				$day == $thisDate['mday'] ? $thisDayStyle : $style, 
				$weekDay < 5 ? "" : " smCalendarWeekend", $content
			);
		}
	...
	/*
	 * $obj->getLargeCalendar($cellContentFunction[, $year, $month])
	 * Date::getLargeCalendar($cellContentFunction, $year, $month)
	 */
	function getLargeCalendar(/* $cellContentFunction [, $year, $month] */)
	{
		switch (func_num_args()) {
		case 3:
			$cellContentFunction = func_get_arg(0);
		...
		$rc = <<<EOT
<table class="lgCalendarTable" width="100%">
<tr>
...
EOT;
		...
		for ($day = 1; $day <= $daysInMonth; $day++, $weekDay = ($weekDay + 1) % 7) {
			...
			if ($cellContentFunction == NULL)
				$content = "<span class='lgCalendarNumber'>${day}</span>";
			else
				$content = $cellContentFunction($year, $month, $day);
							
			$rc .= sprintf(
				"\t<td class='lgCalendarCell %s%s' valign='top'>%s</td>\n", 
				$day == $thisDate['mday'] ? $thisDayStyle : $style, 
				$weekDay < 5 ? "" : " lgCalendarWeekend", $content
			);
		}
